...
export async function GET(request) {
  ...
  const db  = getDb();
  const all = !episode_id || episode_id === "all" || episode_id === "0";
  ...
  const result = await db.execute({
    sql: all
      ? `SELECT c.*,
         ...
         FROM comments c WHERE c.anime_id = ? AND c.parent_id IS NULL
         ORDER BY c.created_at DESC`
      : `SELECT c.*,
         (SELECT COUNT(*) FROM comment_reactions WHERE comment_id = c.id AND type = 1) as likes,
         (SELECT COUNT(*) FROM comment_reactions WHERE comment_id = c.id AND type = 0) as dislikes
         FROM comments c WHERE c.anime_id = ? AND c.episode_id = ? AND c.parent_id IS NULL
         ORDER BY c.created_at DESC`,
    args: all ? [anime_id] : [anime_id, episode_id],
  });
